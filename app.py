...
for idx, row in umsatz_df.iterrows():
        ...
        for key, value in {k.lower(): v for k, v in config["ausgaben"].items()}.items():
            if name.lower() in [x.lower() for x in value]:
                ...
                if shop in ausgaben[key]:
                    ...
                    ausgaben[key][shop] = float(betrag.replace(",", "."))
            # else:
            #     if empfaenger.index(name) == len(empfaenger) - 1:
            #         sonst = "sonstiges"
            #         if not sonst in ausgaben:
            #             ausgaben[sonst] = {}
            #         if first_name in ausgaben[sonst]:
            #             ausgaben[sonst][first_name] += float(betrag.replace(",", "."))
            #         else:
            #             ausgaben[sonst][first_name] = float(betrag.replace(",", "."))
